...
function createTab() {
  if (tabCount >= maxTabs) {
    alert("Maximum of 14 tabs reached.");
    return;
  }
  tabCount++;
...
labelsContainer.addEventListener("click", (event) => {
  ...
  if (closeBtn) {
    const label = closeBtn.parentElement;
    const forId = label.getAttribute("for");

    if (labelsContainer.children.length === 1) return;
    ...
    const inputs = tabsContainer.querySelectorAll(`input[name="${tabGroupName}"]`);
